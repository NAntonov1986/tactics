...
const SKILLS = {
  ...
  */
  ...
  pack_leader: {
    ...
    kind: 'passive',
    trigger: 'aura',
    description: 'Все ПОДЧИНЁННЫЕ юниты группы «Волки» в радиусе 5 клеток получают усиление «Лидер рядом»: +30% Силы (округление вниз от базовой Силы). Сам вожак ауру не получает. Усиление спадает при выходе из радиуса или гибели лидера.',
    tiers: {
  ...
  */
  ...
  joint_hunt_marks: {
    /* Стаки «Совместной охоты» на жертве — отдельный статус-эффект,
       ...
       (натуральное число). В начале хода носителя `onTurnStart`
       снимает эффект целиком (балансная правка 14.05.2026: раньше
       стаки делились пополам с округлением вниз — стая успевала
       нанести слишком много усиленного урона за раунд).
    ...
    kind: 'effect',
    polarity: 'debuff',
    description: 'Стаки совместной охоты. Каждый стак увеличивает урон от носителя пассивки «Совместная охота» на 1. В начале своего хода жертвы все стаки сбрасываются — эффект снимается полностью.',
    onTurnStart(unit, eff) {
      if (!unit || !eff) return;
      ...
      if (before > 0) {
        log(`${who} — «Совместная охота» спадает (было ${before} стак.)`, 'info');
      }
    }
  },
  pack_leader_aura: {
    /* Эффект-аура. На экземпляре хранится `statMods: { str: +N }` —
       N = floor(базовая Сила цели * 0.30) (балансная правка 14.05.2026,
       раньше было ceil). Не тикает (нет remaining), не наследуется
       по длительности — каждый поворот хода refreshPackLeaderAuras
       (core/skills.js) пересчитывает эффект (создаёт / обновляет N /
       снимает) на основании текущего расположения живых лидеров.
    ...
    kind: 'effect',
    polarity: 'buff',
    description: 'Лидер стаи рядом — Сила увеличена на 30% (округление вниз от базовой Силы). Спадает при выходе из радиуса 5 клеток или гибели лидера.'
  },
  frightened: {
